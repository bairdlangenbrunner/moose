...
for chunk in range(chunks_whole):

	print('DOING CHUNK '+str(chunk+1)+' OF '+str(chunks_whole))

	start_idx = chunk_size*chunk
	end_idx = chunk_size*chunk+chunk_size

	print(end_idx-start_idx)
		
	subensembles_hist_x = numpy.zeros((chunk_size, dict_x['nlat'], dict_x['nlon']))
	subensembles_hist_y = numpy.zeros((chunk_size, dict_y['nlat'], dict_y['nlon']))
	subensembles_hist_z = numpy.zeros((chunk_size, dict_z['nlat'], dict_z['nlon']))
	
	model_combinations_subset = model_combinations[start_idx:end_idx,:]

	for i in range(end_idx-start_idx):
		subensembles_hist_x[i,:,:] = numpy.mean(dict_x['fields_hist_mods'][model_combinations_subset[i,:],:,:], axis=0)
		subensembles_hist_y[i,:,:] = numpy.mean(dict_y['fields_hist_mods'][model_combinations_subset[i,:],:,:], axis=0)
		subensembles_hist_z[i,:,:] = numpy.mean(dict_z['fields_hist_mods'][model_combinations_subset[i,:],:,:], axis=0)

	bias_values_subensembles_x = numpy.zeros((end_idx-start_idx))
	bias_values_subensembles_y = numpy.zeros((end_idx-start_idx))
	bias_values_subensembles_z = numpy.zeros((end_idx-start_idx))
	...
	save_filename = 'pareto_front_results_'+DATESTRING+'_NO_LENS_partial_'+str(start_idx)+'-'+str(end_idx)+'.npy'
...
print('DOING FINAL LEFTOVERS')
start_idx = chunk_size*chunks_whole
end_idx = chunk_size*chunks_whole+chunks_leftovers
print(end_idx-start_idx)

subensembles_hist_x = numpy.zeros((end_idx-start_idx+1, dict_x['nlat'], dict_x['nlon']))
subensembles_hist_y = numpy.zeros((end_idx-start_idx+1, dict_y['nlat'], dict_y['nlon']))
subensembles_hist_z = numpy.zeros((end_idx-start_idx+1, dict_z['nlat'], dict_z['nlon']))
